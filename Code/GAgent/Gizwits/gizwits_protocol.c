...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuePH_ERR != cur->valuePH_ERR)
    {
        GIZWITS_LOG("valuePH_ERR Changed\n");
        ret = 1;
    }
    ...
    if(last->valuerssi != cur->valuerssi)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuerssi Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(PH_ERR_BYTEOFFSET,PH_ERR_BITOFFSET,PH_ERR_LEN,(uint8_t *)devStatusPtr,dataPoints->valuePH_ERR);
    ...
    devStatusPtr->valuerssi = gizY2X(rssi_RATIO,  rssi_ADDITION, dataPoints->valuerssi); 

    devStatusPtr->valuePH = exchangeBytes(gizY2X(PH_RATIO,  PH_ADDITION, dataPoints->valuePH)); 
